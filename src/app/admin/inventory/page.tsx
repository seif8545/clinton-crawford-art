...
const EMPTY_FORM = {
  ...
  status: 'available', featured: false, image_url: '',
}
...
export default function InventoryPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (form.image_url && artworkId) {
        await fetch('/api/upload', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ artwork_id: artworkId.toString(), image_url: form.image_url, is_primary: true }),
        })
      }
  ...
  return (
    <div className="p-8">
      ...
      {/* Form modal */}
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-ink/20 backdrop-blur-sm overflow-y-auto py-10 px-4">
          <div className="w-full max-w-2xl glass-card p-8">
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="col-span-2">
                ...
                <input value={form.title} onChange={e => { set('title', e.target.value); if (!editId) set('slug', autoSlug(e.target.value)) }} className={inputCls} />
              </div>
              <div className="col-span-2">
              ...
              <div className="col-span-2">
                ...
                <textarea value={form.description} onChange={e => set('description', e.target.value)} className={inputCls} rows={4} />
              </div>
              <div className="col-span-2">
                <label className={labelCls}>Image URL</label>
                <input value={form.image_url} onChange={e => set('image_url', e.target.value)} className={inputCls}
                  placeholder="https://example.com/image.jpg" />
                <p className="text-xs text-dusk/40 mt-1 font-body">Paste any public image URL (Cloudflare Images, imgbb, etc.)</p>
              </div>
              <div className="col-span-2 flex items-center gap-3">
                ...
                <label htmlFor="featured" className="text-sm text-dusk/60 font-body">Feature on homepage</label>
              </div>
            </div>
      ...
      {loading ? (
        ...
        <div className="glass-card p-16 text-center">
          <p className="font-display text-2xl text-dusk/30 mb-4">No artworks yet</p>
        ...
        <div className="border border-whisper overflow-x-auto">
          <table className="w-full text-sm font-body">
            ...
            <tbody>
              {artworks.map(a => (
                <tr key={a.id} className="border-b border-whisper hover:bg-vellum transition-colors group">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openEdit(a)} className="text-xs text-dusk/50 hover:text-gold px-2 py-1 border border-whisper hover:border-gold/30 transition-colors">Edit</button>
                      <button onClick={() => handleDelete(a.id, a.title)} className="text-xs text-dusk/50 hover:text-blush-deep px-2 py-1 border border-whisper hover:border-blush/30 transition-colors">Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
