...
export default function OrdersPage() {
    ...
    async function load() {
        try {
            ...
            // FIX: Explicitly cast the JSON response to bypass the 'unknown' type error
            ...
            <div className="flex flex-wrap gap-2 mb-8">
                {filters.map(f => (
                    <button key={f} onClick={() => setFilter(f)}
                        className={`text-xs px-4 py-2 tracking-widest uppercase font-body border transition-all capitalize ${filter === f ? 'border-gold/40 text-gold bg-gold/5' : 'border-whisper text-dusk/40 hover:text-ink/60'
                            }`}>
                        {f}
                    </button>
                ))}
