...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    ...
    return {
      title: artwork.title,
      description: artwork.description ?? `${artwork.medium} · ${artwork.dimensions} · ${artwork.series}`,
    }
  } catch {
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-3 mb-5">
                ...
                <span className={`
                  ...
                  ${artwork.status === 'reserved' ? 'badge-reserved' : ''}
                `}>
                  {artwork.status}
                </span>
              </div>
              ...
              <div className="glass-card p-5 text-sm">
                ...
                <p className="text-dusk/60 font-body text-sm mb-3">
                  Questions, installment plans, or international shipping inquiries?
                </p>
                <a
                  href={`mailto:inquiries@clintoncrawfordart.com?subject=Inquiry: ${artwork.title}`}
        ...
        {related.length > 0 && (
          <section className="max-w-7xl mx-auto px-6 py-20 border-t border-whisper mt-10">
            <h2 className="font-display text-4xl text-ink mb-10">
              More from{' '}
              <span className="text-gold italic">{artwork.series ?? 'the Collection'}</span>
            </h2>
