...
interface Props {
  searchParams: { series?: string; status?: string }
}
...
export default async function GalleryPage({ searchParams }: Props) {
  ...
  const activeSeries = searchParams.series ?? 'all'
          ...
          <div className="flex flex-wrap gap-4 items-center border-b border-whisper pb-6">
            ...
            <div className="flex items-center gap-2">
              {[
                ...
                <a
                  key={f.value}
                  href={`/gallery${buildQuery({ ...searchParams, status: f.value === 'all' ? undefined : f.value })}`}
                  className={`text-xs px-4 py-2 tracking-[0.15em] uppercase font-body border transition-all duration-200 ${
                    activeStatus === f.value || (f.value === 'all' && !searchParams.status)
            ...
            {seriesList.length > 0 && (
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <a
                  href={`/gallery${buildQuery({ ...searchParams, series: undefined })}`}
                  className={`text-xs px-4 py-2 tracking-[0.15em] uppercase font-body border transition-all duration-200 ${
                    !searchParams.series
                      ? 'border-gold/40 text-gold bg-gold/5'
                ...
                {seriesList.map(s => (
                  <a
                    key={s}
                    href={`/gallery${buildQuery({ ...searchParams, series: s })}`}
                    className={`text-xs px-4 py-2 tracking-[0.15em] uppercase font-body border transition-all duration-200 ${
                      activeSeries === s
